...
serverCode = serverCode.replace(/9\.3\.14/g, '9.3.15');
fs.writeFileSync('server.js', serverCode);

// Patch package.json
let pkgCode = fs.readFileSync('package.json', 'utf8');
pkgCode = pkgCode.replace(/"version": "9\.3\.14"/, '"version": "9.3.15"');
...
for (const file of htmlFiles) {
    if (fs.existsSync(file)) {
        ...
        html = html.replace(/9\.3\.14/g, '9.3.15');
...
for (const file of jsFiles) {
    if (fs.existsSync(file)) {
        ...
        js = js.replace(/9\.3\.14/g, '9.3.15');
        fs.writeFileSync(file, js);
    }
}
console.log("Version bumped to 9.3.15");
