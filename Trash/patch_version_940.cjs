...
serverCode = serverCode.replace(/9\.3\.15/g, '9.4.0');
fs.writeFileSync('server.js', serverCode);

// Patch package.json
let pkgCode = fs.readFileSync('package.json', 'utf8');
pkgCode = pkgCode.replace(/"version": "9\.3\.15"/, '"version": "9.4.0"');
...
for (const file of htmlFiles) {
    if (fs.existsSync(file)) {
        ...
        html = html.replace(/9\.3\.15/g, '9.4.0');
...
for (const file of jsFiles) {
    if (fs.existsSync(file)) {
        ...
        js = js.replace(/9\.3\.15/g, '9.4.0');
        fs.writeFileSync(file, js);
    }
}
console.log("Version bumped to 9.4.0");
