...
// Inject Maintenance Endpoints
const targetHook = `// ==========================================
// 4. SISTEM PROTEKSI & DEKODE LISENSI
// ==========================================`;

const maintenanceEndpoints = `// ==========================================
// MAINTENANCE & OTA API (Added v9.3.14)
// ==========================================
app.get('/api/maintenance/backup', verifyToken, (req, res) => {
    sysLog('INFO', \`[Maintenance] Backup database requested by \${req.userRole} (\${req.userId})\`, 'SYSTEM');
    res.download(path.join(dataDir, 'nvr.db.json'), \`arch3r_backup_\${Date.now()}.json\`);
});

app.post('/api/maintenance/reboot', verifyToken, (req, res) => {
    sysLog('WARN', \`[Maintenance] System reboot requested by \${req.userRole} (\${req.userId})\`, 'SYSTEM');
...
app.post('/api/maintenance/reset', verifyToken, (req, res) => {
    ...
    if (req.userRole === 'superadmin') {
        ...
        sysLog('WARN', \`[Maintenance] FACTORY RESET by superadmin (\${req.userId})\`, 'SECURITY');
    } else if (req.userRole === 'administrator') {
        dbData.cameras = [];
        dbData.users = [];
        sysLog('WARN', \`[Maintenance] Reset Admin Data by \${req.userId}\`, 'SECURITY');
    } else {
...
app.get('/api/system/ota/check', verifyToken, requireSuperadmin, async (req, res) => {
    try {
        ...
        // Mock fallback if URL is placeholder, otherwise try fetch
        if (otaUrl.includes('YOUR_GITHUB_USERNAME')) {
            return res.json({
                current_version: require('./package.json').version,
                latest_version: '9.4.0',
                changelog: '- Perbaikan bug Minor pada NVR\\n- Optimalisasi UI Mobile\\n- Dukungan Script Otomatis OTA',
...
app.post('/api/system/ota/apply', verifyToken, requireSuperadmin, (req, res) => {
    ...
    setTimeout(() => {
        const cmd = \`git fetch --all && git reset --hard origin/main && git pull && npm install && pm2 restart all\`;
...
console.log("Server maintenance endpoints patched.");
