...
app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', version: 'Archer NVR Ver. 9.4.11' });
});
...
function syncMediaMtxConfig() {
    try {
        ...
        exec('pm2 reload mediamtx || systemctl restart mediamtx || pm2 restart mediamtx', (err, stdout, stderr) => {
            if (err) {
